	...
	<body class="landing">
		<div id="page-wrapper">
							...
							<li>
								...
								<ul>
									...
									<li>
										<a href="#">Submenu</a>
										<ul>
											<li><button onclick="adm();">ADMINISTRADOR</button> </li>
												<li><button onclick="cmp();">comprador</button> </li>
													<li><button onclick="ven();">vendedor</button> </li>
										</ul>
									</li>
								</ul>
							</li>
			...
			<form method='post' action='index.php'>
				<table>
						<tr><td>Mail: </td><td><input type="text" name="email" id='email' required></td></tr>
						<tr><td>Nombre: </td><td><input type="text" name="nombre" id='nombre' required></td></tr>
						<tr>
						<td>Clave: </td><td><input type="password" max="3" size='3' name="clave" id='clave' required></td></tr>
						<tr>
...
//4.- INVOCAMOS AL METODO SOAP, PASANDOLE EL PARAMETRO DE ENTRADA
		$result = $client->call('login',array($_POST['email'],$_POST['clave'],$_POST['nombre']));
		//var_dump($_POST);
		...
		if ($client->fault) {
			...
			if ($err) {//MOSTRAMOS EL ERROR
				...
				if ($result['nombre']==''){
					...
					$_SESSION['loginin']=$result['nombre'];
				...
				$_SESSION['logueado']='entro';
				$_SESSION['nombre']=$result['nombre'];
				$_COOKIE["mail"]=$result['correo'];
				$_COOKIE["nombre"]=$result['nombre'];
  ...
  <script src="//code.jquery.com/ui/1.11.4/jquery-ui.js"></script>
			...
			$(function() {
				...
				function adm(){
					...
					//alert(document.getElementById('email').val);
					$('#email').val('admin@admin.com');
					$('#nombre').val('admin');
					$('#clave').val('321');
				}
				function cmp(){
					$('#email').val('comp@comp.com');
					$('#nombre').val('comprador');
					$('#clave').val('123');
				}
				function ven(){
						$('#email').val('luciano@gmail.com');
					$('#nombre').val('luciano');
					$('#clave').val('321');
				}
				function Modi(id){
