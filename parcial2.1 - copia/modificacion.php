...
//4.- INVOCAMOS AL METODO SOAP, PASANDOLE EL PARAMETRO DE ENTRADA
		...
		if ($btn=='Modificar'){
			//echo 'entro';
			error_reporting(0);
				$resultado = $client->call('ModificarUsuario',array($_POST['nombre'],$_POST['email'],$_POST['clave'],$_POST['dni'],$foto,$_POST['myid']));
			}
		...
		if ($client->fault) {
			...
			if ($err) {//MOSTRAMOS EL ERROR
				...
				echo "<table border='1' width='70%' >
						<tr>
							<th>ID</th><th>Nombre</th><th>Clave</th><th>Email</th><th>DNI</th><th>Foto</th><th>B</th><th>A</th><th>M</th>
						</tr>";
				foreach($result as $user)
				{
					echo "<tr>
							<form method='post' action='modificacion.php' ><td><input required type='text' name='id' value='".$user['id']."' readonly></td>
							...
							<td><input required type='text' name='email' value='".$user['dni']."' readonly></td>
							<td><img src='fotos/".$user['foto']."' alt='Foto Pablo'  height=50 width=50 ></td>
...
<div id='Estoypormodificar'>
		<form method='post' action='modificacion.php' enctype="multipart/form-data" >							
							<table height='100%' width='100%' style="background-color:#000000">
								...
								<tr>
									<td>DNI</td><td><input type='number' required id='dni' name='dni'  style="width:350px" placeholder='Por favor ingrese su DNI'></td>
								</tr>
								<tr>
