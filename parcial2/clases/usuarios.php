...
class usuarios
{
	...
	//	public array PDOStatement::fetchAll ([ int $fetch_style [, mixed $fetch_argument [, array $ctor_args = array() ]]] )
	 ...
	 {
				$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
				$consulta =$objetoAccesoDato->RetornarConsulta("UPDATE `usuarios` SET `nombre`=:nombre,`clave`=:clave,`mail`=:mail WHERE id=:id");
...
//				return $consulta;
	 ...
	 {
				$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
				$consulta =$objetoAccesoDato->RetornarConsulta("INSERT INTO `USUARIOS` (`NOMBRE`,`MAIL`,`CLAVE`) VALUES (:nombre,:mail,:clave)");
				$consulta->bindValue(':nombre',$dato->nombre, PDO::PARAM_STR);
				$consulta->bindValue(':mail',$dato->mail, PDO::PARAM_STR);
				$consulta->bindValue(':clave',$dato->clave, PDO::PARAM_STR);
//				return $consulta;
